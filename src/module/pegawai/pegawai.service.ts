...
export class PegawaiService {
  ...
  async listData(params: PegawaiListDto): Promise<any> {
    ...
    if (params.search) {
      Object.assign(where, {
        [Op.or]: [
          {
            noPegawai: { [Op.substring]: params.search },
          },
          {
  ...
  async reportPegawai(params: ReportDto): Promise<any> {
    const where = {};
    if (params.search) {
      Object.assign(where, {
        [Op.or]: [
  ...
  async count(
    options: Omit<CountOptions<Attributes<Pegawai>>, 'group'>,
  ): Promise<number> {
    return await this.pegawaiModel.count(options);
  }
